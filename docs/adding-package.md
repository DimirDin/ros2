...
### Флаг зависит от платформы

В `cmake_args` доступны подстановки, которые генератор матрицы заменяет
значениями целевой платформы:

| Плейсхолдер | Пример значения |
|---|---|
| `{ros_distro}` | `humble`, `jazzy` |
| `{cuda_arch}` | `87`, `75;86;89` |
| `{platform}` | `jetson-nano-jp7` |
...
Без этого пакет, которому нужен флаг с именем дистрибутива, пришлось бы
описывать отдельным манифестом на каждую платформу. Рабочий пример —
`packages/fast-lio2.yaml`.
